...
* `actions` - (Required) Set of the privilege actions. For a complete list of actions available , see [Custom Role Actions](https://docs.mongodb.com/manual/reference/privilege-actions/)
-> **Note**: The privilege actions available to the Custom Roles API resource represent a subset of the privilege actions available in the Atlas Custom Roles UI.
-> **Note**: `actions` is unordered - MongoDB doesn't preserve the order actions were granted in, and order has no effect on which actions are granted. Listing the same actions in a different order in your configuration than what's already applied will not show as a change.
